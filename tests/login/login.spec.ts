    ...
    if (isSuccess) {
      ...
      Logger.success('Login successful');
        ...
        if (errorMsg === 'Invalid credentials') {
            await expect(loginPage.invliderror).toContainText('Invalid password.');
            Logger.error('Password field error displayed as expected');
        }
        else if (errorMsg === 'Both Required') {
            await expect(loginPage.emailerror).toContainText('The email field is required.');
            await expect(loginPage.passworderror).toContainText('The password field is required.');
